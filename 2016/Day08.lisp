...
(defun apply-instr (instr)
  (let ((rect-instr (search "rect" instr)))
    (cond (rect-instr (multiple-value-bind (col row) (parse-rect-instr instr)
                          (instr-rect col row)))
          (t (multiple-value-bind (instr-type row-col amount) (parse-rotate-instr instr)
